...
export class PointsListComponent {
    ...
    public nextPagePointsCount(): number {
        return this.IDs.filter((val) => val.rowNumber > this.lastPageRow).slice(
            0,
            10
        ).length;
    }
        ...
        if (!this.alreadyFetchingPointsFromDB) {
            ...
            switch (this.filter?.pointSelectionType) {
                ...
                default:
                    ...
                    if (this.filter) {
                        this.filter.slashTag =
                            this.localData.PreviousSlashTagSelected; // how does this relate to getting from route param?
                        if (this.filter.slashTag) {
                            this.pointsService
                                .GetFirstBatchForTag(
                                    this.filter.slashTag,
                                ...
                                .subscribe({
                                    ...
                                    error: (err) => {
                                        this.error = err.error.detail;
                                        this.alreadyFetchingPointsFromDB =
                                            false;
                                    }
                                });
                        }
    ...
    public SelectSpecificPoint(slashTag: string, pointTitle: string): void {
        ...
        this.pointsService.GetSpecificPoint(slashTag, pointTitle).subscribe({
            next: (psr) => {
                console.log(
                    `IS BROWSER: ${isPlatformBrowser(this.platformId)}`
                );

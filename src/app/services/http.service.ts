...
export class HttpService {
  ...
  private RequestHeaders(type: ContentType): HttpHeaders {
    ...
    if (this.localData.GotFreeVoteJwt) { httpHeaders = httpHeaders.append('jwt', this.localData.JWT); }
  ...
  private postFormWithJwtType<T>(url: string, formData: FormData): Observable<any> {
  ...
  get(url: string): Observable<any> {
    ...
    return this.getApiJwt()
      .pipe(
        switchMap(_ => this.getWithJwt(url))
      );
  }
  ...
  post(url: string, data: any): Observable<any> {
    ...
    return this.getApiJwt()
      .pipe(
        switchMap(_ => this.postWithJwt(url, data))
      );
  }
  ...
  uploadProfilePicture(pictureUpload: File): Observable<HttpEvent<ProfilePicture>> {
